#!/usr/bin/env python3
"""
RandomForest - 三角クロス評価（訓練FW以降のみ評価）
...
N_FOLDS = 5
OUTPUT_BASE = ROOT / "experiments/nova_review_acceptance/outputs_rf_triangular_eval"


def run_training(train_fw, output_dir):
    """訓練を実行（特定のFuture Windowで）"""
    cmd = [
        "uv", "run", "python",
        str(ROOT / "experiments/nova_review_acceptance/scripts/train_rf_importants.py"),
        "--train_start", TRAIN_START,
        "--train_end", TRAIN_END,
        "--eval_snapshot", EVAL_SNAPSHOT,
        "--train_fw_start", str(train_fw["fw_start"]),
        "--train_fw_end", str(train_fw["fw_end"]),
        "--eval_fw_start", str(train_fw["fw_start"]),  # 訓練時は同じFWで評価
        "--eval_fw_end", str(train_fw["fw_end"]),
    ...
    if result.returncode != 0:
        logger.error(f"訓練失敗: {train_fw['name']}")
        return False
    
    return True


def evaluate_with_model(model_dir, eval_fw, output_dir):
    """既存モデルで評価（異なるFuture Windowで）"""
    # train_rf_importants.pyは--modelオプションがないので、
    # 訓練済みモデルを読み込むスクリプトを作成する必要がある
    # 今回は簡易的に再訓練+評価FWだけ変更する方式を使用
    
    # モデルディレクトリからtrain_fwを取得
    train_fw_name = model_dir.name.replace("train_", "")
    train_fw = next(fw for fw in FUTURE_WINDOWS if fw["name"] == train_fw_name)
    ...
    cmd = [
        ...
        "--train_fw_start", str(train_fw["fw_start"]),
        "--train_fw_end", str(train_fw["fw_end"]),
        "--eval_fw_start", str(eval_fw["fw_start"]),  # 評価FWを変更
        "--eval_fw_end", str(eval_fw["fw_end"]),
        "--output_dir", str(output_dir),
        "--project", PROJECT,
        "--data_path", DATA_PATH,
        "--n_folds", str(N_FOLDS),
    ...
    if result.returncode != 0:
        logger.error(f"評価失敗: {eval_fw['name']}")
        return False
...
def collect_triangular_eval_results(cross_eval_results):
    ...
    # サマリー表示
    logger.info("\n" + "=" * 80)
    logger.info("🔥 RandomForest - 三角クロス評価 AUC-ROCマトリクス")
    logger.info("=" * 80)
...
def main():
    logger.info("=" * 80)
    logger.info("🔥 RandomForest - 三角クロス評価 🔥")
    logger.info("=" * 80)
    ...
    logger.info(f"K-Fold CV: {N_FOLDS} Folds")
    logger.info("=" * 80)
    ...
    for fw_window in FUTURE_WINDOWS:
        ...
        if model_dir.exists():
            ...
            logger.warning(f"⚠️ モデルディレクトリが見つかりません: {fw_window['name']}")
    ...
    for train_fw_name, model_info in trained_models.items():
        ...
        for eval_fw_idx in range(train_fw_idx, len(FUTURE_WINDOWS)):
            ...
            # 結果を収集
            metrics_path = eval_output_dir / "cv_metrics_summary.json"
            if metrics_path.exists():
                ...
                cross_eval_results.append({
                    ...
                    "auc_roc": metrics.get("mean_auc_roc", 0.0),
                    "auc_pr": metrics.get("mean_auc_pr", 0.0),
                    "f1_score": metrics.get("mean_f1_score", 0.0),
                    "precision": metrics.get("mean_precision", 0.0),
                    "recall": metrics.get("mean_recall", 0.0),
                })
                logger.info(f"✅ AUC-ROC: {metrics.get('mean_auc_roc', 0.0):.4f}")
