#!/usr/bin/env python3
"""
Baseline IRL (No Attention) vs Random Forest 比較ヒートマップ作成
"""
...
output_dir = Path("experiments/nova_review_acceptance/baseline_comparison_heatmaps")
...
# 差分計算 (Baseline IRL - Random Forest)
...
print("=" * 80)
print("Baseline IRL (No Attention) vs Random Forest 比較")
print("=" * 80)
print(f"\nAUC-ROC平均: Baseline IRL={irl_auc_roc_transposed.mean():.4f}, RF={rf_auc_roc_transposed.mean():.4f}, 差={diff_auc_roc.mean():.4f}")
print(f"AUC-PR平均:  Baseline IRL={irl_auc_pr_transposed.mean():.4f}, RF={rf_auc_pr_transposed.mean():.4f}, 差={diff_auc_pr.mean():.4f}")
print(f"F1平均:      Baseline IRL={irl_f1_transposed.mean():.4f}, RF={rf_f1_transposed.mean():.4f}, 差={diff_f1.mean():.4f}")

# 図1: 3つのメトリクス並べて比較 (Baseline IRL vs RF)
fig, axes = plt.subplots(2, 3, figsize=(18, 12))
fig.suptitle('Baseline IRL (No Attention) vs Random Forest - 4×4 Cross-Evaluation', fontsize=16, fontweight='bold')

# AUC-ROC
sns.heatmap(irl_auc_roc_transposed, annot=True, fmt='.3f', cmap='Blues', vmin=0.0, vmax=1.0,
            ax=axes[0, 0], cbar_kws={'label': 'AUC-ROC'})
axes[0, 0].set_title('Baseline IRL (No Attention) - AUC-ROC', fontweight='bold')
...
axes[0, 1].set_title('Baseline IRL (No Attention) - AUC-PR', fontweight='bold')
...
axes[0, 2].set_title('Baseline IRL (No Attention) - F1 Score', fontweight='bold')
...
plt.tight_layout()
plt.savefig(output_dir / 'baseline_comparison_all_metrics.png', dpi=300, bbox_inches='tight')
print(f"\n✅ 保存: {output_dir / 'baseline_comparison_all_metrics.png'}")

# 図2: 差分ヒートマップ (Baseline IRL - Random Forest)
fig, axes = plt.subplots(1, 3, figsize=(18, 6))
fig.suptitle('Performance Difference: Baseline IRL - Random Forest\n(Positive = Baseline IRL Better)',
...
plt.tight_layout()
plt.savefig(output_dir / 'baseline_difference_heatmaps.png', dpi=300, bbox_inches='tight')
print(f"✅ 保存: {output_dir / 'baseline_difference_heatmaps.png'}")

# 図3: サイドバイサイド比較 (AUC-ROCのみ、大きく)
fig, axes = plt.subplots(1, 3, figsize=(20, 6))
fig.suptitle('AUC-ROC Comparison: Baseline IRL vs Random Forest', fontsize=16, fontweight='bold')

sns.heatmap(irl_auc_roc_transposed, annot=True, fmt='.3f', cmap='Blues', vmin=0.0, vmax=1.0,
            ax=axes[0], cbar_kws={'label': 'AUC-ROC'}, annot_kws={'size': 12})
axes[0].set_title(f'Baseline IRL (No Attention)\nMean: {irl_auc_roc_transposed.mean():.4f}', fontweight='bold', fontsize=14)
...
plt.tight_layout()
plt.savefig(output_dir / 'baseline_auc_roc_comparison_large.png', dpi=300, bbox_inches='tight')
print(f"✅ 保存: {output_dir / 'baseline_auc_roc_comparison_large.png'}")

# 統計サマリー保存
summary = {
    'metric': ['AUC-ROC', 'AUC-PR', 'F1 Score'],
    'Baseline_IRL_mean': [irl_auc_roc_transposed.mean(), irl_auc_pr_transposed.mean(), irl_f1_transposed.mean()],
...
print(f"✅ 保存: {output_dir / 'baseline_comparison_summary.csv'}")
