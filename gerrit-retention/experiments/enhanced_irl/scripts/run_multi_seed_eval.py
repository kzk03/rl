...
def prepare_data(df, train_start, train_end, eval_start, eval_end, 
    ...
    # 母集団: 予測期間にレビュー依頼があった人
    eval_future_df = df[(df['request_time'] >= future_start) &
                        (df['request_time'] < future_end)]
    eval_reviewers = eval_future_df['reviewer_email'].unique()
    
    print(f"  対象: {len(eval_reviewers)}人（予測期間にレビュー依頼があった人）")
    ...
    for idx, reviewer in enumerate(eval_reviewers, 1):
        ...
        # ラベル: 予測期間（future_start〜future_end）で活動したか
        future_df = reviewer_df[
            (reviewer_df['request_time'] >= future_start) &
            (reviewer_df['request_time'] < future_end)
        ]
...
def main():
    ...
    output_path = Path(__file__).parent.parent / "results" / "multi_seed_results.csv"
    ...
    for seed in seeds:
        ...
        print(f"\nSeed {seed}: {seed_data['auc'].mean():.4f} ± {seed_data['auc'].std():.4f}")
