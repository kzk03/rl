...
def extract_activity_based_trajectories(
    df: pd.DataFrame,
    ...
    - **正例**：ラベル計算期間内に活動（承諾）があった（レビュー依頼の有無は不問）
    - **負例**：ラベル計算期間内に活動（承諾）がなかった
    - **除外**：なし（全開発者を評価）
    ...
    # ラベル計算期間の活動データ（承諾した活動のみ）
    label_df = df[
        (df[date_col] >= label_start) &
        (df[date_col] < label_end) &
        (df[label_col] == 1)  # 承諾した活動のみ
    ]
    ...
    for reviewer in active_reviewers:
        ...
        # ラベル計算期間の活動（承諾）
        reviewer_label_activity = label_df[label_df[reviewer_col] == reviewer]
        
        # 継続判定：ラベル計算期間内に活動（承諾）があったか
        future_acceptance = len(reviewer_label_activity) > 0
        ...
        for month_start in history_months[:-1]:  # 最後の月を除く
            ...
            # この月のラベル：将来期間に活動（承諾）があったか（レビュー依頼の有無は不問）
            # 全データから該当期間の承諾を確認
            month_future_activity = df[
                (df[date_col] >= future_start) &
                (df[date_col] < future_end) &
                (df[reviewer_col] == reviewer) &
                (df[label_col] == 1)  # 承諾した活動のみ
            ]
            month_label = 1 if len(month_future_activity) > 0 else 0
    ...
    logger.info(f"  スキップ（ラベル期間に活動なし）: {skipped_no_activity}")
    if trajectories:
...
def main():
    ...
    if args.model is None:
        ...
        train_trajectories = extract_activity_based_trajectories(
            df,
