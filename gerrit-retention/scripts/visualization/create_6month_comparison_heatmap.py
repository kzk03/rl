#!/usr/bin/env python3
"""
Create full 4x4 comparison heatmap for 6-month windows version.
Shows IRL vs baselines with 6-month width future windows.
"""
...
irl_matrix = pd.read_csv(
    'importants/review_acceptance_cross_eval_nova_6month/matrix_AUC_ROC.csv',
    index_col=0
)
lr_matrix = pd.read_csv(
    'importants/baseline_nova_6month_windows/logistic_regression/matrix_AUC_ROC.csv',
    index_col=0
)
rf_matrix = pd.read_csv(
    'importants/baseline_nova_6month_windows/random_forest/matrix_AUC_ROC.csv',
    index_col=0
)
...
for ax, (title, data) in zip(axes[1], comparisons):
    if data is not None:
        ...
        ax.axis('off')
        summary_text = f"""
IRL+LSTM vs Baselines (Nova only - 6-Month Windows)
{'='*55}
...
IRL Advantage (All):
  • vs LR:  {'+' if (irl_mean - lr_mean) >= 0 else ''}{(irl_mean - lr_mean):.3f} ({'+' if (irl_mean - lr_mean) >= 0 else ''}{(irl_mean - lr_mean)/lr_mean*100:.1f}%)
  • vs RF:  {'+' if (irl_mean - rf_mean) >= 0 else ''}{(irl_mean - rf_mean):.3f} ({'+' if (irl_mean - rf_mean) >= 0 else ''}{(irl_mean - rf_mean)/rf_mean*100:.1f}%)
...
IRL Advantage (Diagonal+Future):
  • vs LR:  {'+' if (irl_diag_future.mean() - lr_diag_future.mean()) >= 0 else ''}{(irl_diag_future.mean() - lr_diag_future.mean()):.3f} ({'+' if (irl_diag_future.mean() - lr_diag_future.mean()) >= 0 else ''}{(irl_diag_future.mean() - lr_diag_future.mean())/lr_diag_future.mean()*100:.1f}%)
  • vs RF:  {'+' if (irl_diag_future.mean() - rf_diag_future.mean()) >= 0 else ''}{(irl_diag_future.mean() - rf_diag_future.mean()):.3f} ({'+' if (irl_diag_future.mean() - rf_diag_future.mean()) >= 0 else ''}{(irl_diag_future.mean() - rf_diag_future.mean())/rf_diag_future.mean()*100:.1f}%)
...
Future Windows (6-month width):
  • 0-3m training:  0-6m future
  • 3-6m training:  6-12m future
  • 6-9m training:  12-18m future
  • 9-12m training: 18-24m future
...
plt.suptitle('IRL+LSTM vs Baselines: Full 4×4 Evaluation (6-Month Windows)',
             fontsize=16, fontweight='bold', y=0.995)
plt.tight_layout()

# Save figure
output_path = Path('importants/baseline_nova_6month_windows/comparison_heatmaps_full.png')
