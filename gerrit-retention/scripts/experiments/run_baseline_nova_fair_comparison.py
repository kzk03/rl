"""
Baseline Evaluation with Fair Comparison to Nova IRL (max-date方式)

このスクリプトは、Nova IRLと公平な比較を行うため、IRLと同じmax-date方式を実装します。

IRLの制約：
- 月次訓練を使用
- 訓練期間の後半をラベル付けのためだけに使用
- データリーク防止のため、max-date以降は特徴量計算に使用しない

ベースラインの修正：
- IRLと同じmax-dateを設定
- max-date以前のデータのみを特徴量計算に使用
- max-date～train_endの期間をラベル付けのために使用
...
Usage:
    uv run python scripts/experiments/run_baseline_nova_fair_comparison.py \
        ...
        --output importants/baseline_nova_fair_comparison/
"""
...
def split_into_quarters(start: pd.Timestamp, end: pd.Timestamp) -> List[Tuple[pd.Timestamp, pd.Timestamp, int, int]]:
    """
    Split period into 4 quarters and return (start, end, window_start_months, window_end_months)
...
def run_cross_evaluation_fair(
    ...
    train_quarters: List[Tuple[pd.Timestamp, pd.Timestamp, int, int]],
    eval_quarters: List[Tuple[pd.Timestamp, pd.Timestamp, int, int]],
    output_dir: Path
) -> Dict[str, Any]:
    """
    Run cross-evaluation with fair comparison (monthly training - same as IRL).
    """
    print(f"\n{'='*70}")
    print(f"Running Baseline (Fair Comparison - Monthly Training): {baseline_name}")
    print(f"{'='*70}")
    ...
    for train_idx, (_, _, train_window_start, train_window_end) in enumerate(train_quarters):
        train_name = quarter_names[train_idx]
        print(f"\n{'='*60}")
        print(f"Training Period: {train_name}")
        print(f"  Window: {train_window_start}～{train_window_end}ヶ月")
        print(f"{'='*60}")
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"BASELINE EVALUATION (FAIR COMPARISON - max-date方式)")
    print(f"{'='*70}")
    print(f"Baselines: {', '.join(args.baselines)}")
    print(f"Output: {args.output}")
    print(f"\n⚠️  公平な比較のため、IRLと同じmax-date制約を適用します")
    print(f"    - 訓練期間の後半をラベル付けのためだけに使用")
    print(f"    - max-date以前のデータのみを特徴量計算に使用")
    print(f"{'='*70}")
    ...
    print(f"\nTrain quarters:")
    for i, (s, e, ws, we) in enumerate(train_quarters):
    ...
    for baseline_name in args.baselines:
        try:
            results = run_cross_evaluation_fair(
                ...
                train_quarters, eval_quarters, Path(args.output)
            )
