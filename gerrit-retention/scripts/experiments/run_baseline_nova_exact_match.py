"""
Baseline Evaluation with Exact Nova IRL Methodology

This script replicates the exact evaluation methodology used by Nova IRL:
- Train on entire train period data
- Evaluate on entire eval period data
- No train/test split within the same period
- Complete independence between train and eval periods

Usage:
    uv run python scripts/experiments/run_baseline_nova_exact_match.py \
        ...
        --output importants/baseline_nova_exact_match/
"""
...
def split_into_quarters(start: pd.Timestamp, end: pd.Timestamp) -> List[Tuple[pd.Timestamp, pd.Timestamp]]:
    """Split period into 4 quarters"""
...
def extract_trajectories_for_period(
    df: pd.DataFrame,
    ...
    """
    Extract trajectories for a specific period.

    Args:
        df: Full review data
        history_start, history_end: Period for feature extraction
        label_start, label_end: Period for labeling
    ...
    print(f"    Extracted {len(trajectories)} trajectories "
          f"(skipped: {skipped_min_history} min_history, {skipped_no_eval} no_eval)")
...
def run_cross_evaluation_exact_match(
    baseline_name: str,
    df: pd.DataFrame,
    train_quarters: List[Tuple[pd.Timestamp, pd.Timestamp]],
    eval_quarters: List[Tuple[pd.Timestamp, pd.Timestamp]],
    output_dir: Path
) -> Dict[str, Any]:
    """
    Run cross-evaluation matching Nova IRL methodology exactly.
    """
    print(f"\n{'='*70}")
    ...
    # For each train period
    for train_idx, (train_start, train_end) in enumerate(train_quarters):
        train_name = quarter_names[train_idx]
        print(f"\n{'='*60}")
        print(f"Training Period: {train_name} ({train_start.date()} to {train_end.date()})")
        print(f"{'='*60}")

        # Extract train trajectories (label from eval periods)
        # For training, we need to create trajectories that have labels
        # We'll use the first eval period for training labels
        train_label_start, train_label_end = eval_quarters[0]
        ...
        for eval_idx, (eval_start, eval_end) in enumerate(eval_quarters):
            eval_name = quarter_names[eval_idx]
            print(f"\n  Eval Period: {eval_name} ({eval_start.date()} to {eval_end.date()})")

            # Extract eval trajectories
            eval_trajectories = extract_trajectories_for_period(
                df,
                history_start=train_start,  # Use same history period
                history_end=train_end,
                label_start=eval_start,  # But different label period
                label_end=eval_end
            )
...
def main():
    parser = argparse.ArgumentParser(
        description='Baseline evaluation with exact Nova IRL methodology'
    )
    ...
    parser.add_argument('--output', default='importants/baseline_nova_exact_match/')
    ...
    print(f"\n{'='*70}")
    print(f"BASELINE EVALUATION (EXACT NOVA IRL METHODOLOGY)")
    print(f"{'='*70}")
    ...
    # Split into quarters
    train_quarters = split_into_quarters(train_start, train_end)
    eval_quarters = split_into_quarters(eval_start, eval_end)

    print(f"\nTrain quarters:")
    for i, (s, e) in enumerate(train_quarters):
        print(f"  {i*3}-{(i+1)*3}m: {s.date()} to {e.date()}")

    print(f"\nEval quarters:")
    for i, (s, e) in enumerate(eval_quarters):
        print(f"  {i*3}-{(i+1)*3}m: {s.date()} to {e.date()}")
    ...
    for baseline_name in args.baselines:
        try:
            results = run_cross_evaluation_exact_match(
                baseline_name, df, train_quarters, eval_quarters, Path(args.output)
            )
