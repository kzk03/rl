#!/usr/bin/env python3
"""
通常IRLモデルのスナップショット時点評価結果のヒートマップを作成
"""
...
def load_metrics_from_directory(base_dir: str) -> pd.DataFrame:
    """ディレクトリからメトリクスを読み込み、DataFrameに変換"""
    ...
    # 各ディレクトリを探索（train_Xm_eval_Ym形式）
    for eval_dir in base_path.iterdir():
        if not eval_dir.is_dir():
            continue
            
        dir_name = eval_dir.name
        logger.info(f"ディレクトリを処理中: {dir_name}")
        
        # train_Xm_eval_Ym形式から訓練期間と評価期間を抽出
        if '_eval_' in dir_name:
            parts = dir_name.split('_eval_')
            train_period = parts[0]  # train_Xm
            eval_period = dir_name    # train_Xm_eval_Ym
        else:
            logger.warning(f"予期しないディレクトリ名: {dir_name}")
            continue
...
def create_heatmap(data: pd.DataFrame, metric: str, title: str, output_path: str):
    ...
    eval_periods = ['train_0-3m_eval_0-3m', 'train_3-6m_eval_3-6m', 'train_6-9m_eval_6-9m', 'train_9-12m_eval_9-12m']
    
    # インデックスとカラムを再順序付け
    pivot_table = pivot_table.reindex(eval_periods[::-1], columns=periods)
    ...
    plt.title(f'{title} - 通常IRLスナップショット評価', fontsize=16, fontweight='bold')
...
def create_combined_heatmap(data: pd.DataFrame, output_dir: str):
    ...
    for i, (metric, name) in enumerate(zip(metrics, metric_names)):
        ...
        axes[i].set_xlabel('訓練期間', fontsize=10)
        axes[i].set_ylabel('評価期間', fontsize=10)
    ...
    plt.suptitle('通常IRLモデル スナップショット時点評価結果', fontsize=16, fontweight='bold')
    plt.tight_layout()
    
    output_path = os.path.join(output_dir, 'heatmap_combined_normal_irl_snapshot.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def save_matrices_as_csv(data: pd.DataFrame, output_dir: str):
    ...
    for metric in metrics:
        ...
        output_path = os.path.join(output_dir, f'matrix_{metric}_normal_irl_snapshot.csv')
...
def main():
    ...
    # 入力ディレクトリ
    input_dir = "outputs/normal_irl_snapshot_eval"
    
    # 出力ディレクトリ
    output_dir = "outputs/normal_irl_snapshot_eval/heatmaps"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info("=" * 80)
    logger.info("通常IRLモデル スナップショット時点評価ヒートマップ作成開始")
    logger.info(f"入力ディレクトリ: {input_dir}")
    logger.info(f"出力ディレクトリ: {output_dir}")
    logger.info("=" * 80)
    
    # メトリクスデータを読み込み
    data = load_metrics_from_directory(input_dir)
    ...
    for metric, name in metrics_config:
        output_path = os.path.join(output_dir, f'heatmap_{name}_normal_irl_snapshot.png')
    ...
    # 生データも保存
    raw_data_path = os.path.join(output_dir, 'raw_metrics_normal_irl_snapshot.csv')
    data.to_csv(raw_data_path, index=False)
    logger.info(f"生データ保存完了: {raw_data_path}")
    
    logger.info("=" * 80)
    logger.info("通常IRLモデル スナップショット時点評価ヒートマップ作成完了")
    logger.info("=" * 80)
